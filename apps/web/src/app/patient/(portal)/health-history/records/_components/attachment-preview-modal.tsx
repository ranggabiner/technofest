...
export function AttachmentPreviewControl({
  ...
  return (
    <>
      <button
        type="button"
        className="inline-flex min-h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-full px-3 py-2 text-sm font-semibold text-[var(--color-teal-deep)] transition hover:bg-[var(--color-card)] hover:text-[var(--color-midnight)] sm:w-auto"
...
function AttachmentPreviewDialogFallback() {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[color-mix(in_srgb,var(--color-ash)_32%,transparent)] p-3 backdrop-blur-sm sm:p-4">
      <section className="my-4 flex max-h-[calc(100dvh-2rem)] min-h-[520px] w-full max-w-[920px] animate-pulse flex-col overflow-hidden rounded-[18px] border border-[var(--color-stone-surface)] bg-[var(--color-card)] shadow-[0_24px_80px_rgba(18,18,18,0.18),inset_0_0_0_1px_var(--color-stone-surface)]">
