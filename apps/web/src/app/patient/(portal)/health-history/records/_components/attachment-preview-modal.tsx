...
import { Download, Eye, FileText, FileWarning, ImageIcon, X } from "lucide-react";
...
export function AttachmentPreviewControl({
  ...
  return (
    <>
      <button
        type="button"
        className="inline-flex cursor-pointer items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold text-[var(--color-teal-deep)] transition hover:bg-[var(--color-card)] hover:text-[var(--color-midnight)]"
      ...
      {open ? (
        ...
        >
          <section className="my-4 flex max-h-[92vh] w-full max-w-[920px] flex-col overflow-hidden rounded-[18px] border border-[var(--color-stone-surface)] bg-[var(--color-card)] shadow-[0_24px_80px_rgba(18,18,18,0.18),inset_0_0_0_1px_var(--color-stone-surface)]">
            <header className="flex items-start justify-between gap-4 border-b border-[var(--color-stone-surface)] px-5 py-4 sm:px-6">
              <div className="min-w-0">
                <h2
                  ...
                  className="text-[22px] font-semibold leading-tight text-[var(--color-midnight)] sm:text-[26px]"
                >
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="truncate font-semibold text-[var(--color-midnight)]">{filename}</p>
                    <p className="text-xs text-[var(--color-ash)]">{meta}</p>
                  </div>
                </div>
              </div>
              <button
              ...
              <div className="mx-auto grid min-h-[320px] w-full place-items-center rounded-[14px] border border-[var(--color-stone-surface)] bg-[var(--color-card)]">
                {!attachmentFileId ? (
                  ...
                  <iframe
                    src={previewUrl}
                    title={filename}
                    className="h-[60vh] min-h-[320px] w-full rounded-[14px] bg-white"
