...
describe("KYC file validation", () => {
  it("accepts PDF/JPG/PNG up to 5MB", () => {
    expect(validateKycFile({ name: "str.pdf", type: "application/pdf", size: 5 * 1024 * 1024 }).ok).toBe(
      true,
    );
  ...
  it("rejects unsupported or oversized files", () => {
    ...
    expect(validateKycFile({ name: "ktp.pdf", type: "application/pdf", size: 5 * 1024 * 1024 + 1 }).ok).toBe(
      false,
    );
  });
});
