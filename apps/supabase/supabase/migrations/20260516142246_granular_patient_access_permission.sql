...
create table public.access_grant_attachment_permissions (
  grant_id uuid not null references public.access_grants(grant_id) on delete cascade,
  record_id uuid not null references public.scope_1_medical_records(record_id) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (grant_id, record_id)
);

create table public.access_grant_scope2_filters (
  grant_id uuid not null references public.access_grants(grant_id) on delete cascade,
  scope_kind text not null,
  mode text not null,
  window_days integer null,
  session_id uuid null references public.ai_sessions(session_id) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (grant_id, scope_kind),
  constraint access_grant_scope2_filters_scope_kind_check check (scope_kind in ('mental', 'physical')),
  constraint access_grant_scope2_filters_mode_check check (mode in ('last_n_days', 'selected_session')),
  constraint access_grant_scope2_filters_value_check check (
    (
      mode = 'last_n_days'
...
create index access_grant_attachment_permissions_record_idx
on public.access_grant_attachment_permissions(record_id);

create index access_grant_scope2_filters_session_idx
on public.access_grant_scope2_filters(session_id)
where session_id is not null;

alter table public.access_grant_attachment_permissions enable row level security;
alter table public.access_grant_scope2_filters enable row level security;

grant select on public.access_grant_attachment_permissions to authenticated, service_role;
grant select on public.access_grant_scope2_filters to authenticated, service_role;

create policy "authenticated can read visible attachment permissions"
on public.access_grant_attachment_permissions for select to authenticated
using (
  exists (
    select 1
    from public.access_grants ag
    where ag.grant_id = access_grant_attachment_permissions.grant_id
      and ag.patient_id = private.current_patient_id()
  )
  or exists (
    select 1
    from public.access_grants ag
...
create policy "authenticated can read visible scope2 filters"
on public.access_grant_scope2_filters for select to authenticated
using (
  exists (
    select 1
    from public.access_grants ag
    where ag.grant_id = access_grant_scope2_filters.grant_id
      and ag.patient_id = private.current_patient_id()
  )
  or exists (
    select 1
...
begin
  ...
  if coalesce(allow_scope2_mental, false) and mental_filter is not null then
    perform private.validate_access_grant_scope2_filter(target_patient_id, 'mental', mental_filter);
  end if;

  if coalesce(allow_scope2_physical, false) and physical_filter is not null then
...
begin
  ...
  if target_mode = 'last_n_days' then
    target_window_days := nullif(target_filter->>'window_days', '')::integer;
    if target_window_days is null or target_window_days <= 0 then
      raise check_violation using message = 'scope2 recency window is invalid';
    end if;
    return;
  end if;

  if target_mode = 'selected_session' then
    target_session_id := nullif(target_filter->>'session_id', '')::uuid;
    if target_session_id is null then
      raise check_violation using message = 'scope2 session filter is invalid';
    end if;

    if target_scope_kind = 'mental' then
      select count(*)::int
      into session_row_count
      from public.scope_2_mental sm
      where sm.patient_id = target_patient_id
        and sm.session_id = target_session_id;
    elsif target_scope_kind = 'physical' then
      select count(*)::int
      into session_row_count
      from public.scope_2_physical sp
      where sp.patient_id = target_patient_id
        and sp.session_id = target_session_id;
    else
      raise check_violation using message = 'scope2 kind is invalid';
    end if;

    if session_row_count < 1 then
      raise check_violation using message = 'scope2 session does not belong to patient';
    end if;
    return;
  end if;
...
grant execute on function private.validate_access_grant_scope2_filter(uuid, text, jsonb) to authenticated;
