...
export async function POST(
  ...
  if (role.kind !== "doctor") {
    return NextResponse.json({ error: "Akses dokter wajib digunakan" }, { status: 403 });
  }

  const parsed = ragRequestSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: "Format pertanyaan tidak valid" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "AI dokter gagal memproses pertanyaan" }, { status: 500 });
  }
}
