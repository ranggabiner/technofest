...
export async function approveDoctorAction(formData: FormData) {
  ...
  if (!doctorId) throw new Error("doctor_id wajib diisi");
  ...
  if (updateError) throw new Error(updateError.message ?? "Gagal menyetujui dokter");
...
export async function rejectDoctorAction(formData: FormData) {
  ...
  if (!doctorId || !rejectionReason) throw new Error("Alasan penolakan wajib diisi");
